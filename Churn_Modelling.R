...
# 1. Import dataset. 
df <- read.csv('C:\\Users\\PC\\Desktop\\DSA BOOTCAMP\\All Weeks\\week 11\\R\\Churn_Modelling.csv')
df <- df %>% mutate_if(~n_distinct(.)<5,as.factor)
df
df %>% skim()
df %>% glimpse()
class(df$Exited)
...
drops <- c("RowNumber","CustomerId",'Surname')
...
#Explanation to ROC and AUC: According to the illustration derived from the model, we obtained ROC value of approximately 0.78 and AUC value of 0.82. Both figures might be interpreted as almost good results but still could be better. Since the closer the ROC figure to 1 the better is the model at distinguishing different classes.
